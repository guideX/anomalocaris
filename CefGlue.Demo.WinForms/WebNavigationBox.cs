    ...
    public partial class WebNavigationBox : Control {
        ...
        private Button CreateButton(string title, string stockImage, EventHandler action, DockStyle dockStyle = DockStyle.Left) {
            try {
                ...
                if (System.IO.File.Exists(Application.StartupPath + @"\Resources\" + stockImage)) {
                    button.Image = Image.FromFile(Application.StartupPath + @"\Resources\" + stockImage);
                } else {
                    //MessageBox.Show("The file " + Application.StartupPath + @"\Resources\" + stockImage + " does not exist");
                }
